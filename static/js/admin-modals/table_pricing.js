...
async function savePricingPlan() {
    ...
    if (form.checkValidity()) {
        try {
            ...
            // Make API call
            const response = await fetch('/settings/pricing-plans/create/', {
                method: 'POST',
                body: formData,
                headers: {
...
function openEditPricingPlanModal(planId) {
    const modal = document.getElementById('editPricingPlanModal');
    const backdrop = document.getElementById('editPricingPlanBackdrop');
    const content = document.getElementById('editPricingPlanModalContent');
    const form = document.getElementById('editPricingPlanForm');

    if (!modal || !backdrop || !content || !form) {
    ...
    // Load plan data
    fetch(`/settings/pricing-plans/${planId}/`)
        .then(response => response.json())
        .then(data => {
            form.querySelector('[name="name"]').value = data.name;
            form.querySelector('[name="is_default"]').checked = data.is_default;
        ...
        .catch(error => {
            ...
            showNotification('Не удалось загрузить данные тарифа', 'error');
        });
}

async function updatePricingPlan() {
    const form = document.getElementById('editPricingPlanForm');
    const saveBtn = document.querySelector('#editPricingPlanModal button[onclick="updatePricingPlan()"]');
    ...
    if (form.checkValidity()) {
        try {
            ...
            closeModal('editPricingPlanModal');
